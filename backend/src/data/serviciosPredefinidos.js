...
// --- SOLUCIÓN: FUNCIÓN PARA COMBINAR Y FILTRAR DUPLICADOS ---
const combineAndUnique = (...servicesArrays) => {
  const combined = [].concat(...servicesArrays);
  const uniqueServices = new Map();
  combined.forEach(service => {
      if (!uniqueServices.has(service.nombre)) {
          uniqueServices.set(service.nombre, service);
      }
  });
...
// Servicios para veterinarias (combinación sin duplicados)
const serviciosVeterinaria = combineAndUnique(serviciosVeterinario, serviciosCentroVeterinario);

// Servicios para Otros (combinación sin duplicados)
const serviciosOtro = combineAndUnique(serviciosVeterinario, serviciosCentroVeterinario);
